var imChatMain = document.getElementsByClassName("im-chat-main")[0];
...
client.on('MessageFromPeer', ({ text }, peerId) => { // text 为消息文本，peerId 是消息发送方 User ID
    ...
    var chatLiList = chatList.getElementsByTagName("li");
    for (i = 0; i < chatLiList.length; i++) {
        let peer_Id = chatLiList[i].getElementsByClassName("peerId")[0].innerHTML;
        if (peerId == peer_Id) {
            chatLiList[i].getElementsByClassName("mui-ellipsis")[0].innerHTML = text;
            chatLiList[i].getElementsByClassName("time")[0].innerHTML = getNowFormatDate();
            let msgList = document.getElementById(peerId);
            let img;
            users.map(function(v) {
                if (v.peerId == peerId) img = v.img;
            })
            var htmlLi = '<li>' +
            ...
            //追加内容
            msgList.insertAdjacentHTML('beforeEnd', htmlLi);
            return;
        }
    }

    users.push({ peerId: peerId, img: zimu[index] })

    let childNode = document.createElement('li');
    childNode.setAttribute('class', 'mui-table-view-cell mui-media');
    childNode.innerHTML = '<img class="mui-media-object mui-pull-left" src="static/images/' + zimu[index] + '.png">' +
        '<div class="mui-media-body"><text class="peerId">' + peerId + '</text><span class="time">' + getNowFormatDate() + '</span>' +
        '<p class="mui-ellipsis"></p>';
    childNode.getElementsByClassName("mui-ellipsis")[0].innerHTML = text;
    $(childNode).on("tap", function(e) {
        gpeerId = this.getElementsByClassName("peerId")[0].innerHTML;
        $(".nickname").text("guest")
        $("#" + gpeerId).show();
        $(".chat_list").hide();
        $(".chat_content").show();
    ...
    chatList.appendChild(childNode);
    var html = '<ul id=' + peerId + ' style="display:none">' +
        '<li>' +
        '<div class="im-chat-user">' +
        '<img src="/static/images/' + zimu[index] + '.png"/>' +
        '<cite>' + peerId + '</cite>' +
        '</div>' +
        '<div class="im-chat-text">' + text + '</div>' +
        '</li>' +
    ...
    //追加内容
    imChatMain.insertAdjacentHTML('beforeEnd', html);
    index++;
});

client.login({ token: '', uid: 'kefu' }).then(() => {
...
$(function() {
    ...
    $("#msg_text").on({
        ...
        "focus": function() {
            if ($(".more-content").css('display') != 'none') {
                ...
                document.getElementById(gpeerId).scrollTop = document.getElementById(gpeerId).scrollHeight + document.getElementById(gpeerId).offsetHeight;
            }
        }
    });
    ...
    $("#sendMessage").on("tap", function() {
        ...
        var html = '<li class="im-chat-mine">' +
            ...
            '<cite><i>' + getNowFormatDate() + '</i>客服kefu</cite>' +
            '</div>' +
            '<div class="im-chat-text">' + imContent($("#msg_text").val() || '&nbsp;') + '</div>' +
            '</li>';

        //追加内容
        $("#" + gpeerId)[0].insertAdjacentHTML('beforeEnd', html);
        client.sendMessageToPeer({ text: imContent($("#msg_text").val() || '&nbsp;') }, // 符合 RtmMessage 接口的参数对象
            ...
            if (sendResult.hasPeerReceived) {
                ...
                var html = '<li>' +
                    ...
                    '<cite>' + gpeerId + '</cite>' +
                    '</div>' +
                    '<div class="im-chat-text">' + '-----用户不在线-----' + '</div>' +
                    '</li>';
