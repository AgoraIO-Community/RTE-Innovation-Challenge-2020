...
//浮窗宽度
static const CGFloat cFloatingWindowWidth = 120;
//默认缩放动画时间
static const NSTimeInterval cFloatingWindowPathAnimtiontDuration = 0.3;
//浮窗左右两边最小间距
static const CGFloat cFloatingWindowMargin = 10.0;
//红色浮窗隐藏视图宽度
static const CGFloat cFloatingWindowContentWidth = 0.0;
//默认动画时间
static const NSTimeInterval cFloatingWindowAnimtionDefaultDuration = 0.25;
//浮窗上下两边最小间距 非 iPhoneX
static const CGFloat cFloatingWindowTopBottomMargin = 64;
//浮窗上下两边最小间距 iPhoneX
static const CGFloat cFloatingWindowTopBottomMarginIphoneX = 86;
...
+ (void)showWithViewController:(UIViewController *)viewController isPop:(BOOL)isPop {
    ...
    dispatch_once(&onceToken, ^{
        ...
        CGFloat yy = XW_ISIPhoneX ? kScreenH - 83 - 88 - 50 : kScreenH - 49 - 50 - 88;
        xw_floatingWindowView = [[XWFloatingWindowView alloc] initWithFrame:CGRectMake(cFloatingWindowMargin, yy, cFloatingWindowWidth, 50)];
...
+ (void)showWithViewController:(UIViewController *)viewController {
    ...
    dispatch_once(&onceToken, ^{
        ...
        xw_floatingWindowContentView = [[XWFloatingWindowContentView alloc] initWithFrame:CGRectMake(UIScreen.mainScreen.bounds.size.width, UIScreen.mainScreen.bounds.size.height, cFloatingWindowContentWidth, 50)];
    });
    UIWindow *keyWindow = [UIApplication sharedApplication].keyWindow;
    if (!xw_floatingWindowContentView.superview) {
        [keyWindow addSubview:xw_floatingWindowContentView];
...
- (void)setupUI
{
    ...
    self.backgroundColor = [UIColor clearColor];
    UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 112, 40)];
    [self addSubview:imageView];
    NSMutableArray *imgArray = [NSMutableArray array];
    for (int i=1; i<48; i++) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"qi_%d",i]];
...
- (void)toContainerVC
{
    ...
    UINavigationController *navi = app.currentNav;
