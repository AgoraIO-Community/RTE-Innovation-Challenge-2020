...
- (void)viewDidLoad {
    ...
    [self cyl_executeAtDealloc:^{
        [[NSNotificationCenter defaultCenter]removeObserver:self name:GGTEAM_INVITE_CLICK_NOTIFATION object:nil];
        !weakSelf.viewControllerWillDeallocBlock ?: weakSelf.viewControllerWillDeallocBlock(weakSelf);
    }];
    [self.navigationController.navigationBar setBarTintColor:[UIColor whiteColor]];
    [self.navigationController.navigationBar setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:GGTitle_Color,UITextAttributeTextColor,nil]];
      [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(pushToTeam:) name:GGTEAM_INVITE_CLICK_NOTIFATION object:nil];
...
- (void)pushToTeam:(NSNotification *)note
{
    NSDictionary *dic = note.object;
    NSLog(@"================push");
    NSString *teamId = dic[@"teamId"];
    GGTeamModel *model = [GGTeamModel new];
    model.objectId = teamId;
   
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否确认加入此房间?" preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确定加入" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"加入新房间");
...
#warning 此处不注释掉会引起crash,加入两个群聊的时候会引起此问题
...
- (void)messageCell:(LCCKChatMessageCell *)messageCell didTapLinkText:(NSString *)linkText linkType:(MLLinkType)linkType {
    switch (linkType) {
        case MLLinkTypeURL: {
            NSString *urlstring = linkText;
            NSString *hostUrl = GGHOST_URL;
            if ([urlstring hasPrefix:GGHOST_URL]) {
                //说明是房间消息
                NSString *teamCode = [urlstring substringFromIndex:hostUrl.length+1];
                NSString *key = [GGEncryTool xorEncryptorDecrypt:teamCode];
                if ([key integerValue ] > 200) {
                    AVQuery *query = [AVQuery queryWithClassName:DB_TEAM];
                    [query whereKey:@"teamCode" equalTo:[NSNumber numberWithInteger:[key integerValue]]];
...
//                    GGSimpleWebViewController *webViewController = [[GGSimpleWebViewController alloc] init];
