...
struct ChannelMediaOptions {
  ...
  */
  ...
  /**
   * Determines whether to publish the video of the multi-camera track.
   * - true: Publish the video track of the multiple camera capturers.
   * - false: (Default) Do not publish the video track of the camera capturers.
   */
  ...
  */
  ...
  ChannelMediaOptions& operator=(const ChannelMediaOptions& channelMediaOptions) {
    if (this != &channelMediaOptions) {
      ...
      publishSecondaryCameraTrack = channelMediaOptions.publishSecondaryCameraTrack;
...
class IRtcEngine : public agora::base::IEngineBase {
  ...
  */
  ...
  virtual int startScreenCapture(void* mediaProjectionPermissionResultData,
                                 const ScreenCaptureParameters& captureParams) = 0;
  /**
   * Create local video/audio track for media player
   *
   * @note
   * This method applies to Android only.
   * @param mediaPlayerSource -- IMediaPlayerSource
   *
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
  virtual int createLocalMediaPlayerTrack(const agora_refptr<rtc::IMediaPlayerSource>& mediaPlayerSource) = 0;
  /**
   * Destroy local video/audio track for media player
   *
   * @note
   * This method applies to Android only.
   * @param id -- id of media player
   *
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
    virtual int destroyLocalMediaPlayerTrack(int id) = 0;
#endif
