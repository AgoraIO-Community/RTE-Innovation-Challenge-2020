...
/// @cond (!Linux)
  ...
  */
  ...
  /**
    * Creates a local video track object with a secondary camera capturer and returns the pointer.
    *
    * Once created, this track can be used to send video data captured by the the secondary camera.
    * Noted:
    * Before createSecondaryCameraVideoTrack, you must enable primary camera by calling createCameraVideoTrack()
    *
    * @param videoSource The pointer to the camera capturer: \ref agora::rtc::ICameraCapturer "ICameraCapturer".
    *                                     You must provide the same camera capturer in createCameraVideoTrack
    *
    * @return
    * - The pointer to \ref rtc::ILocalVideoTrack "ILocalVideoTrack", if the method call succeeds.
    * - A null pointer, if the method call fails.
    */
   virtual agora_refptr<rtc::ILocalVideoTrack> createSecondaryCameraTrack(
       agora_refptr<rtc::ICameraCapturer> videoSource) = 0;
