...
struct VideoCanvas {
  ...
  int renderSource;
  // Used for secondary camera track preview, see publishSecondaryCameraTrack.
  bool isSecondaryCameraView;

  VideoCanvas() : view(NULL), renderMode(media::base::RENDER_MODE_HIDDEN), uid(0), userId(NULL), priv(NULL),
                  renderSource(media::base::RENDER_SOURCE_CAMERA) {}
  VideoCanvas(view_t v, int m, uid_t u)
      : view(v), renderMode(m), uid(u), userId(NULL), priv(NULL),
        renderSource(media::base::RENDER_SOURCE_CAMERA), isSecondaryCameraView(false) {}
  VideoCanvas(view_t v, int m, user_id_t u)
      : view(v), renderMode(m), uid(0), userId(u), priv(NULL),
        renderSource(media::base::RENDER_SOURCE_CAMERA), isSecondaryCameraView(false) {}
  VideoCanvas(view_t v, int m, int s, user_id_t u)
      : view(v), renderMode(m), uid(0), userId(u), priv(NULL),
        renderSource(s), isSecondaryCameraView(false) {}
};
