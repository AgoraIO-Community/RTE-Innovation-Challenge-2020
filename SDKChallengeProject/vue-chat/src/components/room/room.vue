<template>
  <div class="app-main">
    <li-header @headerLeftEvent="loginEvent" @headerRightEvent="configEvent" :config="headerConfig">
        <div slot="title" class="header-title" @click="headCenterEvent">娱乐交友<span style="fontSize: 12px;">▼</span>
        </div>
    </li-header>
...
export default {
  ...
  data () {
    return {
      ...
      onlineUserList: [],
      messageList: [], // type = 1 提示信息     type = 2 对方内容     type = 3 我发送内容
      inputValue: '',
      connectState: false,
      client: null,
      channel: null,
  ...
  created () {
    this.client = AgoraRTM.createInstance('8f864f9ba96f438aaedd066e1d53746f');
  ...
  methods: {
    connectEvent () {
      ...
      this.client.login({ uid: this.userInfo.userId }).then(() => {
        // me.onlineUserList = obj.onlineUserList;
        me.connectState = true; // 登录状态
        me.headerConfig.left = me.userInfo.userImg.toString();
        this.channel = this.client.createChannel('default');
        this.channel.on('ChannelMessage', ({ text }, senderId) => {
          let obj = JSON.parse(text)
          me.messageList.push({type: 2, msg: obj.msg, msgUser: obj.user});
        });
        this.channel.on('MemberJoined', (senderId) => {
          me.messageList.push({type: 1, msg: '用户 ' + senderId + ' 加入聊天'});
        });
        this.channel.on('MemberLeft', (senderId) => {
          me.messageList.push({type: 1, msg: '用户 ' + senderId + ' 退出聊天'});
        });
        this.channel.join().then(() => {
          /* 加入频道成功的处理逻辑 */
          this.messageList.push({type: 1, msg: '用户 ' + this.userInfo.userId + ' 加入聊天', msgUser: this.userInfo});
        }).catch(error => {
          console.log(error)
        });
      }).catch(err => {
    ...
    getUserId () {
      return 'u' + (new Date().getTime() + '' + Math.floor(Math.random() * 100000 + 100));
    },
    configEvent () {
    ...
    sendEvent () {
      ...
      if (this.inputValue.length > 0) {
        if (this.connectState) {
          let data = {msg: this.inputValue, user: this.userInfo}
