...
- (void)requestForceSingleSignOnAuthorizationWithCallback:(LCCKRequestAuthorizationBoolResultBlock)callback {
    ...
    NSString *title = @"当前账号已在其它设备登录，请问是否强制登录？强制登录会踢掉当前已经登录的设备。";
...
//    NSString *forceOpenActionTitle = LCCKLocalizedStrings(@"ok") ?: @"确认";
    NSString *forceOpenActionTitle = @"确认";
...
- (void)conversation:(AVIMConversation *)conversation membersAdded:(NSArray<NSString *> *)clientIds byClientId:(NSString *)clientId
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:GGLCCKNotificationMessageMemberAdded object:userInfo];
}
...
- (void)conversation:(AVIMConversation *)conversation membersRemoved:(NSArray<NSString *> * _Nullable)clientIds byClientId:(NSString * _Nullable)clientId
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:GGLCCKNotificationMessageMemberRemoved object:userInfo];
}
