...
- (void)refresh {
    self.freshing = YES;
    [[LCCKConversationListService sharedInstance] findRecentConversationsWithBlock:^(NSArray *conversationsArr, NSInteger totalUnreadCount, NSError *error) {
        
#pragma mark - Lofter修改(会话列表不显示房间会话)start
        NSMutableArray *conversations = [NSMutableArray arrayWithCapacity:0];
        for(AVIMConversation *conversation in conversationsArr)
        {
...
//            {
             [self.conversationListViewController filterAVIMError:error];
...
//            }
#pragma mark - Lofter修改(会话列表不显示房间会话)End
        };
