...
public class UDTEventHandlerMy : MonoBehaviour
{
    ...
    public void BuildNewTarget()
    {
            ...
            m_FrameQuality == ImageTargetBuilder.FrameQuality.FRAME_QUALITY_HIGH)
        {
            ...
            //    Debug.Log(ImageTargetTemplate.GetSize().x);
            m_TargetBuildingBehaviour.BuildNewTarget(targetName, ImageTargetTemplate.GetSize().x*2f);
        }
        else
        {
    ...
    public void RemoveTarget(string nameId)
    {

        // Deactivates the dataset first
        m_ObjectTracker.DeactivateDataSet(m_UDT_DataSet);
        //   int tarId = int.Parse(nameId);
        // Destroy the oldest target if the dataset is full or the dataset
        // already contains five user-defined targets.
        //  if (m_UDT_DataSet.HasReachedTrackableLimit() || m_UDT_DataSet.GetTrackables().Count() >= MAX_TARGETS)
        //   {
        string dd = "-" + nameId;
        IEnumerable<Trackable> trackables = m_UDT_DataSet.GetTrackables();
        Trackable toMove = null;
        foreach (Trackable trackable in trackables)
        {
  //          Debug.Log("tarId:" + dd + "trackable.ID" + trackable.Name);
            if (dd == trackable.Name)
            {
                toMove = trackable;
                break;
            }
        ...
        if (toMove != null)
        {
     //       Debug.Log("Destroying oldest trackable in UDT dataset: " + toMove.Name);
            m_UDT_DataSet.Destroy(toMove, true);
        }
        //    }
        // Activate the dataset again
        m_ObjectTracker.ActivateDataSet(m_UDT_DataSet);
